...
async def send_start(message: Message):
    await message.answer("Привет! Я ваш личный финансовый помощник. Выберите пункт меню:", reply_markup=keyboards)
...
async def registration(message: Message):
    ...
    # Берём первый попавшийся результат
    user = cursor.fetchone()
    if user:
        await message.answer("Вы уже зарегистрированы!")
    else:
        cursor.execute('''INSERT INTO users (telegram_id, name) VALUES (?, ?)''', (telegram_id, user_name))
        conn.commit()
...
async def exchange_rates(message: Message):
    url = "https://v6.exchangerate-api.com/v6/97364f37b0e89567f1fa67a4/latest/USD"
    try:
